...
//						parameter.put(item.getFieldName(), item.getString());
				...
				insertNotice.setMemberId(id);
				insertNotice.setNoticeTitle(title);
				insertNotice.setNoticeContent(content);
				
				insertNotice.setAttachList(new ArrayList<NoticeAttachDTO>());
				
				AdminNoticeService noticeService = new AdminNoticeService();
				
				List<NoticeAttachDTO> list = insertNotice.getAttachList();
				...
				if(title.length() == 0 || content.length() == 0) {
					...
					int result = noticeService.insertNotice(insertNotice);
