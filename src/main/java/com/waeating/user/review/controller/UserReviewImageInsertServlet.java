...
/**
 * Servlet implementation class UserReviewImageInsertServlet
 */
@WebServlet("/user/review/image/insert")
public class UserReviewImageInsertServlet extends HttpServlet {
...
if(ServletFileUpload.isMultipartContent(request)) {
			...
			String fileUploadDirectory = rootLocation + "/resources/upload/review/origin";
//			String copynailDirectory = rootLocation + "/resources/upload/review/copy";
			
			File directory = new File(fileUploadDirectory);
//			File directory2 = new File(copynailDirectory);
...
//				String userId = member.getId();
				...
				String reviewContent = parameter.get("reviewContent");
				int recordNo = Integer.parseInt(parameter.get("recordNo"));
				int comNo = Integer.parseInt(parameter.get("comNo"));
				
				int ratings = Integer.parseInt(parameter.get("ratings"));
				
			
						
				/* 서비스를 요청할 수 있도록 BoardDTO에 담는다. */
				ReviewDTO insertReview = new ReviewDTO();
				insertReview.setReviewContent(reviewContent);
				insertReview.setReviewAttach(new ArrayList<ReviewAttachDTO>());
				insertReview.setRecordNo(recordNo);
				insertReview.setComNo(comNo);
				insertReview.setRatings(ratings);
				
				ReviewService reviewService = new ReviewService();
				
				List<ReviewAttachDTO> list = insertReview.getReviewAttach();
				for(int i = 0; i < fileList.size(); i++) {
					Map<String, String> file = fileList.get(i);
					
					ReviewAttachDTO tempFileInfo = new ReviewAttachDTO();
...
//					tempFileInfo.setThumbnailPath(file.get("thumbnailPath"));
				...
				if ( reviewContent.length() == 0) {
					...
					out.println("<script>alert('리뷰를 다시 입력하세요.'); location.href='list';</script>");
